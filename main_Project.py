...
def method_rectangles(f, a, b,*arg):
    """Подщет интеграла методом прямоугольников
                ...
                Выход: ans * shag - площадь под функцией f
                     """
    ans=0
    n=10000
    shag=(b-a)/n
...
def method_trapecia(f, a, b,*arg):
    """Подщет интеграла методом трапеций
                    ...
                    Локальные переменные:
                          ...
                          n - кол-во значений функций
                          shag - на сколько изменяется переменная прикаждом измерении
                    Выход: result * shag - площадь под функцией f
                         """
    result=0
    if str(f)[10:14]=='step':
...
def method_Simson(f, a, b,*arg):
    """Подщет интеграла методом Симпсона
                        ...
                        Выход: ans2*(b-a)/3/10000 - площадь под функцией f в приближенни параболы
                             """
    if str(f)[10:14]=='step':
...
def method_Ghaus(f, a, b, *arg):
    """Подщет интеграла методом Симпсона
                            ...
                            Выход: (b - a) * integ - площадь под функцией f вычисленная по квадратурной формуле
                                 """
...
def method_Monte_Karlo(f, a, b, *arg):
    """Подщет интеграла методом Симпсона
                                ...
                                Выход: itog - площадь под функцией f вычислення методом монте-карло
                                     """
    s = []
    x = 0
    y = 0
...
def clever_grafik(f,method):
    """Подщет интеграла методом прямоугольников
                    ...
                    Локальные переменные:
                          ...
                          y - список разниц между методом от когорого вызван график и функциией вычисления интеграла из питона
                    Выход: график погрешностей
                         """
    t = []
    y1 = []
